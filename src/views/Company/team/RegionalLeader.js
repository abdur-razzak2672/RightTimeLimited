    ...
    <div>
      <section className="section-padding--top">
        <div className="container">
          <h3 className="blog-card-one__title blog-details__title text-center" style={{ color: "#17224d", marginTop: "-100px" }}>Regional Leaders</h3>
                ...
                <Row>
                  {leaders.map((leader) => (
                    <Col md={4} sm={6} xs={12} key={leader.id}>
                      <article className={`material-card mt-5 ${leader.colorClass}`}>
                        <a onClick={() => handleShow(leader.id)} href='javascript:void(0)'>
                          <h2 className="h21 text-light">
                            <span>{leader.name}</span>
                            <strong>
                              {leader.id === 'melinda' && <i className="fa fa-fw fa-star"></i>}
                              {leader.position}
                            </strong>
                          </h2>
                        </a>
      ...
      {leaders.map((leader) => (
        <Modal key={leader.id} size="lg" show={showModal === leader.id} onHide={handleClose}>
            ...
            <div className="position-relative">
              <div className="float-end ms-4 mb-3" style={{ width: '200px' }}>
                ...
                <div className="text-center mt-2">
                  <i style={{ fontSize: "30px" }} className="fab fa-linkedin text-primary"></i>
                </div>
              </div>
