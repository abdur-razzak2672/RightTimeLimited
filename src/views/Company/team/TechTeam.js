...
function TechTeam() {
  ...
  const teamMembers = [
    ...
    },
    {
      ...
      certifications: 'NAHAR HOLDS - SWIFT PRO, ITIL, LEAD AUDITOR- ISO 9001 (QMS), ACUNETIX PRO'
    },
    {
      id: 'tinne',
    ...
    <div>
      <section className="">
        <div className="container">
          <h3 className="blog-card-one__title blog-details__title text-center" style={{ color: "#17224d" }}>Tech Team</h3>
                ...
                <Row>
                  {teamMembers.map((member) => (
                    <Col md={4} sm={6} xs={12} key={member.id}>
                      <article className={`material-card mt-5 ${member.colorClass}`}>
                        <a onClick={() => handleShow(member.id)} href='javascript:void(0)'>
                          <h2 className="h21 text-light">
                            <span>{member.name}</span>
                            <strong>
                              {member.id === 'tohid' && <i className="fa fa-fw fa-star"></i>}
                              {member.position}
                            </strong>
                          </h2>
                        </a>
                        ...
                        <div className="mc-footer">
                          ...
                          <a href={member.linkedin || "/facebook"} style={{ backgroundColor: "#0A66C2", paddingLeft: "15px", borderRadius: "15px" }} className="fab fa-linkedin"></a>
      ...
      {teamMembers.map((member) => (
        <Modal key={member.id} size="lg" show={showModal === member.id} onHide={handleClose}>
